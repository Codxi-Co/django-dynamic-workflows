...
from django_workflow_engine.choices import WorkflowStatus, WorkflowAttachmentStatus, ActionType
...
from django_workflow_engine.handlers import get_handler_for_instance, WorkflowApprovalHandler
...
class TestCompleteWorkflowIntegration:
    ...
    def test_complete_workflow_lifecycle(self):
        ...
        finance_department = Department.objects.create(name='Finance Department', company=company)
        ...
        hr_stage1 = Stage.objects.create(
            ...
            stage_info={
                'approval_config': {
                    ...
                    'role_selection_strategy': RoleSelectionStrategy.CONSENSUS.value
                }
            }
        )
        ...
        with patch('django_workflow_engine.utils.build_approval_steps') as mock_build_steps:
        ...
        with patch('django_workflow_engine.services.move_to_next_stage') as mock_move:
            with patch('django_workflow_engine.services.trigger_workflow_event') as mock_trigger:
        ...
        with patch('django_workflow_engine.services.execute_action_function') as mock_execute:
        ...
        with patch('django_workflow_engine.services.reject_workflow_stage') as mock_reject:
            with patch('django_workflow_engine.services.trigger_workflow_event') as mock_trigger:
        ...
        config = WorkflowConfiguration.objects.get(
            content_type=content_type
        )
    ...
    def test_workflow_with_custom_actions(self):
        ...
        custom_action = WorkflowAction.objects.create(
            ...
            order=0,
            parameters={
                'custom_message': 'Custom action executed',
                'test_mode': True
            }
        )
        ...
        with patch('django_workflow_engine.services.execute_action_function') as mock_execute:
            ...
            assert call_args[1]['function_path'] == 'tests.test_integration.custom_test_action'
            assert call_args[1]['parameters']['custom_message'] == 'Custom action executed'
