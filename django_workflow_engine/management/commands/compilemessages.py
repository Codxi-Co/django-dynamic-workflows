...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(f'Compiling messages for django-workflow-engine in locales: {", ".join(locales)}')
        )
            ...
            if os.path.exists(po_file):
                ...
                try:
                    ...
                    if os.path.exists(mo_file):
                        self.stdout.write(
                            self.style.SUCCESS(f'✓ Successfully compiled {locale} messages')
                        )
                    else:
                        self.stdout.write(
                            self.style.WARNING(f'⚠ Warning: {mo_file} not found after compilation')
                        )
                ...
                self.stdout.write(
                    self.style.WARNING(f'⚠ No translation file found for {locale}: {po_file}')
                )

        self.stdout.write(
            self.style.SUCCESS('✓ Translation compilation completed!')
        )